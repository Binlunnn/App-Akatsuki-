(function () {
  const FB_RENEW_URL = "https://www.facebook.com/share/1DbvnhhG87/?mibextid=wwXIfr"; 

  // ====== DANH MỤC KEY ======
  window.UCHIHA_KEYS = {
    "TrGiang": { plan:{days:30},  note:"Menu make by trgiang" },
    "VuTruongGiang": { plan:{days:30}, note:"VuTruongGiangvippro" }
  };
  ...
  const API = {
    ...
    _fail(keyRaw, message, reason){
      ...
      if (fails[keyId] >= FAIL_LIMIT) {
        ...
        rec.locked = true; rec.reason = "too_many_fail"; rec.lockAt = now();
        binds[keyId] = rec; save(STORE_BIND, binds);
        return { ok:false, msg:`Key bị khoá do nhập sai ${FAIL_LIMIT} lần. Nhấn “Gia hạn/Liên hệ”.`, reason:"too_many_fail", renew:FB_RENEW_URL };
      }
      save(STORE_FAILS, fails);
